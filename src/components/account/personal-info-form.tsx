...
export function PersonalInfoForm({ user, locale = "en" }: { user: { firstName: string; lastName: string; email: string; phone: string | null }; locale?: Locale }) {
  const router = useRouter();
  const t = getDictionary(locale).personalInfoForm;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input id="phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+970 59 000 0000" dir="ltr" />
      </div>
