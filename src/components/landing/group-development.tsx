...
export function GroupDevelopment() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-border bg-primary py-20 text-primary-foreground sm:py-28">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_0%,hsl(var(--accent)/0.18),transparent_60%)]" />
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-14 grid grid-cols-2 gap-6 sm:grid-cols-5">
          {t.stages.map((label, i) => {
            const Icon = ICONS[i];
            return (
              <Reveal key={label} delay={i * 80} className="flex flex-col items-center gap-3 text-center">
                <div className="relative flex h-14 w-14 items-center justify-center rounded-full bg-white/10 shadow-glow">
